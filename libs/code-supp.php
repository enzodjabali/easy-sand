<?php
	...
	if(isset($_POST['access']))
	{
		if(isset($_POST['code']) AND !empty($_POST['code']))
		{
			...
			function genGoSupp($longueurGoSupp = 5)
			{
			return substr(str_shuffle(str_repeat($xGoSupp='0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', ceil($longueurGoSupp/strlen($xGoSupp)) )),1,$longueurGoSupp);
			}
			$rlGoSupp = genGoSupp();

			shell_exec('cp ../../libs/supp-wcode.php '.$accesscode.'/ && mv '.$accesscode.'/supp-wcode.php '.$accesscode.'/'.$rlGoSupp.'.php');
			header('Location:'.$accesscode.'/'.$rlGoSupp.'.php');
		}
	}
?>
<!DOCTYPE html>
